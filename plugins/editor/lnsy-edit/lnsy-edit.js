...
class LNSYEdit extends HTMLElement {
  connectedCallback(){
    ...
    this.json_editor = document.createElement('json-edit');
  ...
  saveData(){
    ...
    let file_name =  json_data["dtrm-id"];
  ...
  loadData(content, metadata){
    if(metadata["dtrm-id"]){
      this.setAttribute('dtrm-id', metadata["dtrm-id"]);
    } else {
      metadata["dtrm-id"] = this.getAttribute('fdtrmile-id');
      if(metadata["dtrm-id"] === null){

        metadata["dtrm-id"] = crypto.randomUUID();
      }
    }
  ...
  loadMarkdown(markdown){
    ...
    if(metadata["dtrm-id"]){
      ...
      metadata["dtrm-id"] = this.getAttribute("dtrm-id");
    }
